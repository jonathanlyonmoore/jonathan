...
exports.reply = function (r) {
	if (this.bot == null) {
		this.bot = new JonathanBot(false);
	} 
...
JonathanBot.prototype.reset = function() {
	...
	for (var k=0; k<this.JonathanKeywords.length; k++) {
		this.lastchoice[k]=[];
		var rules=this.JonathanKeywords[k][2];
...
JonathanBot.prototype._init = function() {
	...
	if ((this.JonathanSynons) && (typeof this.JonathanSynons == 'object')) {
		for (var i in this.JonathanSynons) synPatterns[i]='('+i+'|'+this.JonathanSynons[i].join('|')+')';
	}
	// check for keywords or install empty structure to prevent any errors
	if ((!this.JonathanKeywords) || (typeof this.JonathanKeywords.length == 'undefined')) {
		this.JonathanKeywords=[['###',0,[['###',[]]]]];
	}
	...
	for (var k=0; k<this.JonathanKeywords.length; k++) {
		var rules=this.JonathanKeywords[k][2];
		this.JonathanKeywords[k][3]=k; // save original index for sorting
	...
	this.JonathanKeywords.sort(this._sortKeywords);
	// and compose regexps and refs for pres and posts
	JonathanBot.prototype.pres={};
	JonathanBot.prototype.posts={};

	if ((this.JonathanPres) && (this.JonathanPres.length)) {
		var a=new Array();
		for (var i=0; i<this.JonathanPres.length; i+=2) {
			a.push(this.JonathanPres[i]);
			JonathanBot.prototype.pres[this.JonathanPres[i]]=this.JonathanPres[i+1];
		}
	...
	if ((this.JonathanPosts) && (this.JonathanPosts.length)) {
		var a=new Array();
		for (var i=0; i<this.JonathanPosts.length; i+=2) {
			a.push(this.JonathanPosts[i]);
			JonathanBot.prototype.posts[this.JonathanPosts[i]]=this.JonathanPosts[i+1];
		}
	...
	if ((!this.JonathanQuits) || (typeof this.JonathanQuits.length == 'undefined')) {
		this.JonathanQuits=[];
	}
	// done
...
JonathanBot.prototype.transform = function(text) {
	...
	for (var i=0; i<parts.length; i++) {
		...
		if (part!='') {
			...
			for (var q=0; q<this.JonathanQuits.length; q++) {
				if (this.JonathanQuits[q]==part) {
					this.quit=true;
			...
			for (var k=0; k<this.JonathanKeywords.length; k++) {
				if (part.search(new RegExp('\\b'+this.JonathanKeywords[k][0]+'\\b', 'i'))>=0) {
...
JonathanBot.prototype._execRule = function(k) {
	var rule=this.JonathanKeywords[k];
	var decomps=rule[2];
	var paramre=/\(([0-9]+)\)/;
	for (var i=0; i<decomps.length; i++) {
		...
		if (m!=null) {
			...
			var rpl=reasmbs[ri];
			if (this.debug) alert('match:\nkey: '+this.JonathanKeywords[k][0]+
				'\nrank: '+this.JonathanKeywords[k][1]+
				'\ndecomp: '+decomps[i][0]+
				'\nreasmb: '+rpl+
				'\nmemflag: '+memflag);
...
JonathanBot.prototype._postTransform = function(s) {
	...
	s=s.replace(/\s{2,}/g, ' ');
	s=s.replace(/\s+\./g, '.');
	if ((this.JonathanPostTransforms) && (this.JonathanPostTransforms.length)) {
		for (var i=0; i<this.JonathanPostTransforms.length; i+=2) {
			s=s.replace(this.JonathanPostTransforms[i], this.JonathanPostTransforms[i+1]);
			this.JonathanPostTransforms[i].lastIndex=0;
		}
	}
...
JonathanBot.prototype._getRuleIndexByKey = function(key) {
	for (var k=0; k<this.JonathanKeywords.length; k++) {
		if (this.JonathanKeywords[k][0]==key) return k;
	}
	return -1;
}
...
JonathanBot.prototype.getFinal = function() {
	...
	if (!this.JonathanFinals) return '';
	return this.JonathanFinals[Math.floor(Math.random()*this.JonathanFinals.length)];
}

JonathanBot.prototype.getInitial = function() {
	if (!this.JonathanInitials) return '';
	return this.JonathanInitials[Math.floor(Math.random()*this.JonathanInitials.length)];
}
