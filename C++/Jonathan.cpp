/*
    ...
    Description:
		These program is an Eliza like chatterbot,bots like Jonathan are the results
		...
		on the original chatterbot Jonathan that was written by Joseph Weizenbaum which means
		...
		knowledge is known as "Case Base Reasoning" or CBR. A very good example of an Jonathan
...
void Jonathan::preProcessResponse() {
...
void Jonathan::memorise_input() {
	m_sSymbol = "@";
	extract_suffix();
	memory.push(m_sSuffix);
}

bool Jonathan::similar_response() const {
...
inline bool Jonathan::bot_repeat() const {
...
inline void Jonathan::select_response() {
	shuffle(response_list, response_list.size());
	m_sResponse = response_list[0];
}

void Jonathan::add_response(vstring v) {
	int size = v.size();
...
inline void Jonathan::dump_data() {
...
inline void Jonathan::extract_suffix() {
...
void Jonathan::findSymbol(std::string str) {
	m_sSymbol.erase();
...
inline bool Jonathan::isGoodKey(const std::string key, const std::string bkey, int pos, int bestp) const{
...
void Jonathan::verify_context(vstring vContext) { 
...
void Jonathan::verify_keyword_boundary(std::string keyWord, int pos) {
...
inline void Jonathan::handle_repetition() {
...
void Jonathan::handle_user_repetition() {
	++m_nUserRepeatCount;
...
void Jonathan::handle_short_input() {
	++m_nShortInputCount;
...
void Jonathan::handle_unknown_sentence() {
...
void Jonathan::handle_null_input() {
	response_list = nullResponse;
}

void Jonathan::remind_prev_subject() {
...
// transposes a word
bool Jonathan::transpose( std::string &str ) const {
	std::string _str = " " + str + " ";
...
void Jonathan::transpose_sentence( std::string &str ) {
	std::string thisWord = "";
	std::string tempStr = "";
	vstring stringToken;
	tok.setDelim(" !?,-");
...
void Jonathan::correct_sentence( std::string &str ) const {
	std::string string1 = "";
	std::string string2 = "";
	std::string _str = " " + str + " ";
...
void Jonathan::print_current_data() {
...
void Jonathan::print_database_content() {
...
void Jonathan::save_unknown_sentences() {
...
void Jonathan::saveTopic(const vstring vList, const std::string sSymbol) {
...
void Jonathan::saveComment(const std::string comment) {
	vstring vComment;
	vComment.push_back(comment);
	saveTopic(vComment, "");
}

void Jonathan::saveTransposTable() {
	for(int i = 0; i < m_nTransPosNum; ++i) {
...
void Jonathan::save_log(std::string str) {
	if(str == "Jonathan") {
...
int main() {
	//std::cout << "Chatterbot Jonathan v2.0 Copyright(c) 2005 - 2006 Gonzales Cenelia\n" << std::endl;
	std::cout << std::endl;
	...
	try {
		...
		Jonathan.save_unknown_sentences();
	}
